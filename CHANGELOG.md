...
### Fixed
- **UI Dark Mode**: 修复了“选择季数”弹窗在暗色模式下文字显示为黑色的问题，以及添加影视区域存在的白色背景问题。
- **Search Layout**: 修复了搜索结果卡片中“添加”按钮在长标题下被挤出或遮挡的问题。统一了卡片高度与布局，将按钮固定于底部，提升视觉一致性。
- **Global Styles**: 全局审计并修复了多个组件（`ResultList`, `FolderScan`, `SeasonSelector` 等）中的硬编码颜色，确保暗色模式下的体验一致性。

## [1.0.5] - 2026-01-20

### Added
- **季数选择功能**: 在添加剧集时新增季数选择弹窗，支持 TMDB 多季数据的独立管理与添加。
- **特别篇支持**: 支持获取并显示 TMDB 中的 Season 0（特别篇），并自动规范化命名。
- **UI 增强**: 搜索结果卡片新增类型角标（电影/剧集）以及剧情简介展示，提升辨识度。

### Changed
- **搜索源迁移**: 移除豆瓣搜索支持，全面转向 TMDB 接口以获得更稳定的元数据。
- **命名规范化**: 剧集命名格式统一为 `剧名第X季`，移除了原有的括号格式。

### Fixed
- **数据丢失修复**: 修复了由于数据库架构升级导致旧数据不可见的问题，实现了 `season_number` 字段的平滑迁移。
- **构建修复**: 修复了生产环境打包时的 TypeScript 编译错误。
